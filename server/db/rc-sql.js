...
const supervisorSql = (db) => {
    ...
    const getRcBriefWithCondition = (obj) => {
        ...
        const sql = `Select  SQL_CALC_FOUND_ROWS * from registrationcandidate where sID = '${sID}' and ${keyword ? `(name like '%${keyword}%' or email  like '%${keyword}%' or phone like '%${keyword}%' or rcID = '${keyword}') ` : "1=1"} ${!isNil(gender) ? `and gender = '${Number(gender)}'` : "and 1=1"} ${orderBy ? `Order By ${orderBy} ${orderAsc ? "ASC" : "DESC"}` : ""} ${(skip && take) ? `limit ${take} offset ${skip}` : ""}`;
