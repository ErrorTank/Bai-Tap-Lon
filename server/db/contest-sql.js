...
const contestSql = (db) => {
  ...
  const getContestBriefWithCondition = (obj) => {
    ...
    const sql = `Select  SQL_CALC_FOUND_ROWS c.contestID, c.contestName as contestName, c.fee, s.name as subjectName, ol.name as orgLocation  ,  Count(mix.candidateID) as count from contest c inner join subject s on s.subjectID = c.subjectID inner join orglocation ol on ol.orgLocationID = c.orgLocationID left join (select candidateID, contestID from examdatecandidate ec inner join examdate e on e.examDateID = ec.examDateID) mix  on mix.contestID = c.contestID  where ${keyword ? `c.contestName like '%${keyword}%' or c.contestID = '${keyword}'` : "1=1"}  ${!isNil(canSeeResult) ? `and c.canSeeResult = '${Number(canSeeResult)}'` : "and 1=1"} group by c.contestID  ${orderBy ? `Order By ${orderBy} ${orderAsc ? "ASC" : "DESC"}` : ""} ${(skip && take) ? `limit ${take} offset ${skip}` : ""}`;
    console.log(sql);
    return new Promise((resolve, reject) => {
